...
export const paytm = async(req, res) => {
    ...
    PaytmChecksum.generateSignature(JSON.stringify(paytmParams.body), process.env.PAYTM_MERCHANT_KEY).then(function(checksum){
        ...
        post_res.on('end', function(){
          ...
          console.log({obj});
          let data = {env: process.env.PAYTM_HOSTNAME, mid:process.env.PAYTM_MID, amount:amount,orderid:orderId,txntoken:obj.body.txnToken}
...
export const paytmCallback = (req, res) => {
  ...
  req.on('end', () => {
    ...
    var verifyChecksum =  PaytmChecksum.verifySignature(postbodyjson, process.env.PAYTM_MERCHANT_KEY, checksum);
    if(verifyChecksum) {
